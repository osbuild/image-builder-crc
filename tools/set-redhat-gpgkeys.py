...
DEFAULT_KEYS = r"""-----BEGIN PGP PUBLIC KEY BLOCK-----
...
def update_gpgkeys_in_file(file_path, gpgkey):
    try:
        ...
        dist_name = data['distribution'].get('name', '')
        if not dist_name.startswith('rhel'):
            return False
        
        modified = False
        
        for arch_key in ['x86_64', 'aarch64']:
            if arch_key not in data:
                continue
            
            arch_data = data[arch_key]
            if 'repositories' not in arch_data:
                continue
            
            for repo in arch_data['repositories']:
                if repo.get('id') in ['baseos', 'appstream']:
                    repo['gpgkey'] = gpgkey
                    modified = True
...
def main():
    gpgkey = DEFAULT_KEYS.rstrip('\n')
    
    distributions_dir = Path('distributions')
    if not distributions_dir.exists():
        print(f"Error: distributions directory not found at {distributions_dir}", file=sys.stderr)
        return 1
    
    json_files = list(distributions_dir.rglob('*.json'))
    
    if not json_files:
        print("Warning: No JSON files found in distributions directory", file=sys.stderr)
        return 0
    
    updated_count = 0
    for json_file in sorted(json_files):
        if update_gpgkeys_in_file(json_file, gpgkey):
