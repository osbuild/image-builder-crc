...
func TestGetComposeSBOMs(t *testing.T) {
	var sbomDocRaw = json.RawMessage(`{
  ...
  "relationships": [
	...
	err := json.Unmarshal(sbomDocRaw, &sbomDoc)
	require.NoError(t, err)

	testCases := []struct {
		name               string
		composerStatusCode int
		composerResponse   *composer.ComposeSBOMs
		expectedResponse   *v1.ComposeSBOMsResponse
		queryParams        *v1.GetComposeSBOMsParams
		expectedStatusCode int
	}{
		{
			name:               "404 - SBOMs not found",
			composerStatusCode: http.StatusNotFound,
			expectedStatusCode: http.StatusNotFound,
		},
		{
			name:               "No SBOMs",
			composerStatusCode: http.StatusOK,
			composerResponse:   &composer.ComposeSBOMs{},
			expectedResponse: &v1.ComposeSBOMsResponse{
				Meta: v1.ListResponseMeta{Count: 0},
				Links: v1.ListResponseLinks{
					First: "/api/image-builder/v1.0/composes/%s/sboms?offset=0&limit=100",
					Last:  "/api/image-builder/v1.0/composes/%s/sboms?offset=0&limit=100",
				},
				Data: []v1.ImageSBOM{},
			},
		...
		{
			name:               "SBOMs - happy path",
			composerStatusCode: http.StatusOK,
			composerResponse: &composer.ComposeSBOMs{
				Items: [][]composer.ImageSBOM{
					{
						composer.ImageSBOM{
							PipelineName:    "build",
							PipelinePurpose: composer.Buildroot,
							Sbom:            sbomDoc,
							SbomType:        composer.Spdx,
						},
						composer.ImageSBOM{
							PipelineName:    "os",
							PipelinePurpose: composer.Image,
							Sbom:            sbomDoc,
							SbomType:        composer.Spdx,
						},
					},
			...
			expectedResponse: &v1.ComposeSBOMsResponse{
				Meta: v1.ListResponseMeta{Count: 2},
				Links: v1.ListResponseLinks{
					First: "/api/image-builder/v1.0/composes/%s/sboms?offset=0&limit=100",
					Last:  "/api/image-builder/v1.0/composes/%s/sboms?offset=0&limit=100",
				},
				Data: []v1.ImageSBOM{
					{
						PipelineName:    "build",
						PipelinePurpose: v1.Buildroot,
						Sbom:            sbomDoc,
						SbomType:        v1.Spdx,
					},
					{
						PipelineName:    "os",
						PipelinePurpose: v1.Image,
						Sbom:            sbomDoc,
						SbomType:        v1.Spdx,
					},
				},
		...
		{
			name:               "SBOMs - custom query params #1",
			composerStatusCode: http.StatusOK,
			composerResponse: &composer.ComposeSBOMs{
				Items: [][]composer.ImageSBOM{
					{
			...
			queryParams: &v1.GetComposeSBOMsParams{
				Limit:  common.ToPtr(1),
				Offset: common.ToPtr(0),
			},
			expectedResponse: &v1.ComposeSBOMsResponse{
				Meta: v1.ListResponseMeta{Count: 2},
				Links: v1.ListResponseLinks{
					First: "/api/image-builder/v1.0/composes/%s/sboms?offset=0&limit=1",
					Last:  "/api/image-builder/v1.0/composes/%s/sboms?offset=1&limit=1",
				},
				Data: []v1.ImageSBOM{
					{
						PipelineName:    "build",
		...
		{
			name:               "SBOMs - custom query params #2",
			composerStatusCode: http.StatusOK,
			composerResponse: &composer.ComposeSBOMs{
				Items: [][]composer.ImageSBOM{
					{
		...
		{
			name:               "SBOMs - too many image requests",
			composerStatusCode: http.StatusOK,
			composerResponse: &composer.ComposeSBOMs{
				Items: [][]composer.ImageSBOM{
					{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			apiSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				if tc.composerStatusCode != http.StatusOK {
					w.WriteHeader(tc.composerStatusCode)
					fmt.Fprintf(w, "%d during tests", tc.composerStatusCode)
					return
				}

				err := json.NewEncoder(w).Encode(tc.composerResponse)
				require.NoError(t, err)
			}))
			defer apiSrv.Close()
			...
			var result v1.ComposeSBOMsResponse
			// Get API response and compare
			requestURL := fmt.Sprintf("%s/api/image-builder/v1/composes/%s/sboms", srv.URL, id)
			if tc.queryParams != nil && (tc.queryParams.Limit != nil || tc.queryParams.Offset != nil) {
				var limitPart, offsetPart string
				if tc.queryParams.Limit != nil {
					limitPart = fmt.Sprintf("limit=%d", *tc.queryParams.Limit)
				}
				if tc.queryParams.Offset != nil {
					offsetPart = fmt.Sprintf("offset=%d", *tc.queryParams.Offset)
				}
				requestURL = fmt.Sprintf("%s?%s", requestURL, strings.Join([]string{limitPart, offsetPart}, "&"))
			}
			respStatusCode, body := tutils.GetResponseBody(t, requestURL, &tutils.AuthString0)
			require.Equal(t, tc.expectedStatusCode, respStatusCode)
			if tc.expectedStatusCode == http.StatusOK {
				err = json.Unmarshal([]byte(body), &result)
				require.NoError(t, err)
				// replacet the compose ID in the expected response links
				tc.expectedResponse.Links.First = fmt.Sprintf(tc.expectedResponse.Links.First, id)
				tc.expectedResponse.Links.Last = fmt.Sprintf(tc.expectedResponse.Links.Last, id)
