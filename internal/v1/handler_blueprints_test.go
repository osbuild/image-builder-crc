...
func TestLintBlueprint(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			lintErrors: []string{
				"package required-by-compliance required by policy is not present",
				"service enabled-required-by-compliance required as enabled by policy is not present",
				"service masked-required-by-compliance required as masked by policy is not present",
			},
		},
		{
			blueprint: v1.BlueprintBody{
		...
		},
		{
			...
			lintErrors: []string{
				"mountpoint /tmp required by policy is not present",
				"mountpoint /var required by policy is not present",
				"kernel command line parameter '-compliance' required by policy not set",
			},
		},
		{
			blueprint: v1.BlueprintBody{
...
func TestFixupBlueprint(t *testing.T) {
	...
	for idx, c := range cases {
		...
		respStatusCode, _ = tutils.PostResponseBody(t, fmt.Sprintf("%s/api/image-builder/v1/blueprints/%s/fixup", srv.URL, bpID), &tutils.AuthString0)
		require.Equal(t, http.StatusCreated, respStatusCode)
