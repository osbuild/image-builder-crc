...
func TestFixupBlueprint(t *testing.T) {
	...
	for idx, c := range cases {
		...
		name := fmt.Sprintf("bp-fixup-%d", idx)
		createBody := map[string]any{
			"name":         name,
			"description":  "fixup test",
			"distribution": c.blueprint.Distribution,
		...
		require.NoError(t, err)
		// If a snapshot existed before, fixup must be idempotent
		if snapshotBeforeFixup != nil {
			require.Equal(t, snapshotBeforeFixup, afterFixup.ServiceSnapshots)
		} else {
			require.NotNil(t, afterFixup.ServiceSnapshots)
		}
