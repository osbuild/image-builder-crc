...
func TestFixupBlueprint(t *testing.T) {
	...
	for idx, c := range cases {
		...
		respStatusCode, _ = tutils.PostResponseBody(t, fmt.Sprintf("%s/api/image-builder/v1/experimental/blueprints/%s/fixup", srv.URL, bpID), nil)
		require.Equal(t, http.StatusCreated, respStatusCode)
...
func TestBlueprintComplianceSnapshot(t *testing.T) {
	...
	require.NotEmpty(t, serviceSnapshots.Compliance.PolicyBlueprint)

	var policyData map[string]interface{}
	err = toml.Unmarshal([]byte(serviceSnapshots.Compliance.PolicyBlueprint), &policyData)
	require.NoError(t, err)
